...
Func GetLangText($Key)
	...
	If IsDeclared("cmbLanguage") Then
		...
		$ReturnStr = IniRead(@ScriptDir & "\Gabenk\languages\" & $CurrLangSel & ".ini", "general", $Key, "")
		If $ReturnStr = "" Then
			$ReturnStr = IniRead(@ScriptDir & "\Gabenk\languages\English.ini", "general", $Key, "")
		EndIf
...
;~ 		EndIf
	Else
		$ReturnStr = IniRead(@ScriptDir & "\Gabenk\languages\" & $StartupLanguage & ".ini", "general", $Key, "")
		If $ReturnStr = "" Then
			$ReturnStr = IniRead(@ScriptDir & "\Gabenk\languages\English.ini", "general", $Key, "")
		EndIf
;~ 		If IsDeclared("txtLog") And IsDeclared("statLog") Then
...
Func PopulateLanguages()
	$searchfile = FileFindFirstFile(@ScriptDir & "\Gabenk\languages\*.ini")
	$txtLang = ""
	While True
